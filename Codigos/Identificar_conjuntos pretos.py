...
#    gray = cv2.cvtColor( black_result, cv2.COLOR_BGR2GRAY)
#    _, gray = cv2.threshold(gray, 0, 255, cv2.THRESH_BINARY)
#    cv2.imshow("BINARIO",gray)
#    return gray
    
def trancking(frame, gray):
...
def main():
    ...
    while True:
        ...
        if validacao:           
            ...
            gray = lineanize_black_hsv(frame)
            # lineanize_black_bgr(frame)
            # gray = lineanize_black_bgr(frame)
            frame = trancking(frame, gray)
            
            cv2.imshow('Pincipal', frame)            
